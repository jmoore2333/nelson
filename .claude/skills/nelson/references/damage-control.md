# Damage Control

Consult the specific procedure that matches the situation.

| Situation | Procedure |
|---|---|
| Agent unresponsive, looping, or producing no useful output | `damage-control/man-overboard.md` |
| Session interrupted (context limit, crash, timeout) | `damage-control/session-resumption.md` |
| Completed task found faulty, other tasks are sound | `damage-control/partial-rollback.md` |
| Mission cannot succeed, continuing wastes budget | `damage-control/scuttle-and-reform.md` |
| Issue exceeds current authority or needs clarification | `damage-control/escalation.md` |
